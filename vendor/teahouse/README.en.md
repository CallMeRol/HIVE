...
<img src="build/icons/pantry-logo-icon.png" alt="Hive logo" width="120" height="120" />
...
Start Hive on computers connected to the same local network and they discover each other automatically. Open a peer to chat or transfer files directly. No cloud account, central server, telemetry, or Internet access is involved. Hive is designed for corporate intranets, isolated networks, and laboratories where external connectivity is limited or prohibited.

## Why Hive

Moving a file or sending a short message inside an office network should be effortless. Existing tools often miss at least one requirement: commercial solutions require licensing fees, Neiwangtong provides only Windows binaries without support for domestic UOS or macOS, and iptux presents a steeper learning curve for regular office users. Hive aims for an open-source, serverless, zero-configuration experience across Windows 7, domestic UOS deployments, Linux, and macOS.
...
```bash
chmod +x Hive-*.AppImage
./Hive-*.AppImage
```

**macOS** — Open the `.dmg` and drag Hive into Applications. An unsigned or unnotarized intranet build may require approval under System Settings → Privacy & Security. You can also remove the quarantine attribute in terminal:
...
1. Start Hive on each device connected to the LAN. Peers on the same subnet appear automatically in the left contact list.
...
The discovery sequence takes inspiration from IP Messenger, while Hive uses its own UTF-8 JSON protocol and does not claim wire compatibility with legacy ipmsg clients.
...
- **Physical LAN trust model** — Hive adopts the unencrypted LAN transmission model of classic tools like IP Messenger and iptux, intended for operation within trusted intranet perimeters and isolated laboratory networks.
...
|---|---|
| Windows | `%APPDATA%\Hive\logs` |
| Linux / UOS / Kylin | `${XDG_CONFIG_HOME:-~/.config}/Hive/logs` |
| macOS | `~/Library/Application Support/Hive/logs` |
