...
describe('tray unread integration', () => {
  ...
  it('macOS 设置菜单栏数字、Dock 角标和 tooltip', async () => {
    ...
    expect(tray.setToolTip).toHaveBeenCalledWith('Hive（12 条未读）')
  ...
  it('macOS 未读清零时清除菜单栏数字和 Dock 角标', async () => {
    ...
    expect(tray.setToolTip).toHaveBeenCalledWith('Hive')
  ...
  it('Windows 设置任务栏 overlay 数字，并启动托盘闪烁兜底', async () => {
    ...
    expect(tray.setToolTip).toHaveBeenCalledWith('Hive（99+ 条未读）')
  ...
  it('Windows 未读清零时清除 overlay，并停止后续闪烁', async () => {
    ...
    expect(tray.setToolTip.mock.calls[tray.setToolTip.mock.calls.length - 1]).toEqual(['Hive'])
  ...
  it('Linux 尝试系统角标，并使用托盘闪烁兜底', async () => {
    ...
    expect(win.setOverlayIcon).not.toHaveBeenCalled()
    expect(tray.setToolTip).toHaveBeenCalledWith('Hive（5 条未读）')
    expect(tray.setImage).toHaveBeenCalledTimes(1)
  ...
  it('Linux 未读清零后恢复常规图标并停止闪烁', async () => {
    ...
    expect(tray.setToolTip).toHaveBeenLastCalledWith('Hive')
  })
