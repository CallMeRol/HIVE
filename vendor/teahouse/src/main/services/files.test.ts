...
describe('FilesService 群聊媒体', () => {
  ...
  it('发送更新包 offer 不进入聊天与普通传输视图', async () => {
    ...
    tmpDirs.push(dir)
    const pkgPath = join(dir, 'Hive-0.28.0-linux-amd64.deb')
    ...
    expect(JSON.parse(row.files)).toMatchObject({
      name: 'Hive-0.28.0-linux-amd64.deb',
      savedPath: pkgPath,
      purpose: 'update'
    })
    ...
    expect(messenger.sent[0]).toMatchObject({
      ...
      env: {
        ...
        payload: {
          op: 'offer',
          rootName: 'Hive-0.28.0-linux-amd64.deb',
          purpose: 'update'
        }
      }
    })
  ...
  it('更新包 offer 不进入聊天与接收目录，并尝试自动 accept', async () => {
    ...
    messenger.emit(
      ...
      makeEnvelope<FileCtlPayload>(MSG_TYPES.fileCtl, 'node-bob', {
        ...
        seq: 1,
        total: 1,
        files: [{ fileId: 'f-1', path: 'Hive-0.28.0-linux-amd64.deb', size: 1024 }],
        totalSize: 1024,
        fileCount: 1,
        rootName: 'Hive-0.28.0-linux-amd64.deb',
        purpose: 'update'
    ...
    expect(JSON.parse(row!.files)).toMatchObject({
      name: 'Hive-0.28.0-linux-amd64.deb',
      purpose: 'update',
      savedPath: join(updateDir, 'Hive-0.28.0-linux-amd64.deb')
    })
    expect(messenger.sent[0]).toMatchObject({
      peerId: 'node-bob',
