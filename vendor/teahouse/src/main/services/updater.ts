...
export function matchesUpdatePackageName(
  ...
  if (platform === 'win') {
    if (arch) return name === `Hive-${version}-win-${arch}-setup.exe`
    return (
      name === `Hive-${version}-win-x64-setup.exe` ||
      name === `Hive-${version}-win-ia32-setup.exe` ||
      name === `Hive-${version}-win-arm64-setup.exe`
    )
  }
  if (platform === 'linux') {
    if (arch) {
      ...
      return name === `Hive-${version}-linux-${debArch}.deb`
    }
    return (
      name === `Hive-${version}-linux-amd64.deb` ||
      name === `Hive-${version}-linux-arm64.deb`
    )
  }
  return false
}
