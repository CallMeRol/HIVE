...
describe('自更新请求复核与本地包查找', () => {
  ...
  it('按平台与版本从候选目录查找安装包', () => {
    ...
    tmpDirs.push(dir)
    writeFileSync(join(dir, 'Hive-0.28.0-linux-x86_64.AppImage'), 'appimage')
    writeFileSync(join(dir, 'Hive-0.28.0-linux-amd64.deb'), 'deb')
    writeFileSync(join(dir, 'Hive-0.28.0-linux-arm64.deb'), 'arm64-deb')
    writeFileSync(join(dir, 'Hive-0.28.0-win-x64-portable.exe'), 'portable')
    writeFileSync(join(dir, 'Hive-0.27.9-win-x64-setup.exe'), 'old')
    writeFileSync(join(dir, 'Hive-0.28.0-win-x64-setup.exe'), 'exe')
    writeFileSync(join(dir, 'Hive-0.28.0-win-ia32-setup.exe'), 'ia32-exe')

    expect(findLocalUpdatePackage({ dirs: [dir], version: '0.28.0', platform: 'linux', arch: 'x64' })).toBe(
      join(dir, 'Hive-0.28.0-linux-amd64.deb')
    )
    expect(findLocalUpdatePackage({ dirs: [dir], version: '0.28.0', platform: 'linux', arch: 'arm64' })).toBe(
      join(dir, 'Hive-0.28.0-linux-arm64.deb')
    )
    expect(findLocalUpdatePackage({ dirs: [dir], version: '0.28.0', platform: 'win', arch: 'x64' })).toBe(
      join(dir, 'Hive-0.28.0-win-x64-setup.exe')
    )
    expect(findLocalUpdatePackage({ dirs: [dir], version: '0.28.0', platform: 'win', arch: 'ia32' })).toBe(
      join(dir, 'Hive-0.28.0-win-ia32-setup.exe')
    )
  ...
  it('安装包名必须精确匹配版本、平台与架构', () => {
    expect(matchesUpdatePackageName('Hive-0.28.0-win-x64-setup.exe', '0.28.0', 'win', 'x64')).toBe(true)
    expect(matchesUpdatePackageName('Hive-0.28.0-win-ia32-setup.exe', '0.28.0', 'win', 'ia32')).toBe(true)
    expect(matchesUpdatePackageName('Hive-0.28.0-win-x64-setup.exe', '0.28.0', 'win', 'ia32')).toBe(false)
    expect(matchesUpdatePackageName('Hive-0.28.0-linux-amd64.deb', '0.28.0', 'linux', 'x64')).toBe(true)
    expect(matchesUpdatePackageName('Hive-0.28.0-linux-arm64.deb', '0.28.0', 'linux', 'ia32')).toBe(false)
    expect(matchesUpdatePackageName('Hive-0.28.0-linux-arm64.deb', '0.28.0', 'linux', 'arm64')).toBe(true)
    expect(matchesUpdatePackageName('copy-Hive-0.28.0-linux-amd64.deb', '0.28.0', 'linux', 'x64')).toBe(false)
    expect(matchesUpdatePackageName('Hive-0.28.0-linux-amd64.deb.bak', '0.28.0', 'linux', 'x64')).toBe(false)
    expect(matchesUpdatePackageName('Hive-0.28.1-win-x64-setup.exe', '0.28.0', 'win', 'x64')).toBe(false)
    expect(matchesUpdatePackageName('Hive-0.28.0-mac-arm64.dmg', '0.28.0', 'mac', 'arm64')).toBe(false)
  })
})
...
describe('UpdateRequestGate', () => {
  it('只消费有效期内来源、包名和大小全部匹配的一次 offer', () => {
    ...
    expect(gate.consume('node-b', 'Hive-0.28.0-linux-amd64.deb', 1024, 1_100)).toBe(false)
    expect(gate.consume('node-a', 'Hive-0.28.0-linux-arm64.deb', 1024, 1_100)).toBe(false)
    expect(gate.consume('node-a', 'Hive-0.28.0-linux-amd64.deb', 512 * 1024 * 1024 + 1, 1_100)).toBe(false)
    expect(gate.consume('node-a', 'Hive-0.28.0-linux-amd64.deb', 1024, 1_100)).toBe(true)
    expect(gate.consume('node-a', 'Hive-0.28.0-linux-amd64.deb', 1024, 1_100)).toBe(false)
  })
  ...
  it('授权过期，且旧请求取消不会清掉新请求', () => {
    ...
    expect(gate.consume('node-b', 'Hive-0.29.0-win-x64-setup.exe', 1024, 2_100)).toBe(true)

    gate.begin({ nodeId: 'node-c', version: '0.30.0', platform: 'win', arch: 'x64' }, 3_000)
    expect(gate.consume('node-c', 'Hive-0.30.0-win-x64-setup.exe', 1024, 123_000)).toBe(false)
    ...
    expect(gate.consume('node-d', 'Hive-0.31.0-win-x64-setup.exe', 1024, 4_100)).toBe(false)
  })
})
