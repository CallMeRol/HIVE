  ...
  private onGroupCtl(env: Envelope, rinfo?: RemoteInfo): void {
    ...
    if (payload.op === 'info') {
      ...
      if (applied) {
        ...
        // Hive #53：本机被「别人移出」了（不是自己 leaveGroup）——被移出的节点据此收工退出进程。
        if ((local?.members.includes(this.deps.selfId) ?? false) && !incoming.members.includes(this.deps.selfId)) {
          this.emit('removed', incoming.groupId, incoming.updatedBy)
        }
      }
      return
    }
