...
describe('transfer 数据面回环', () => {
  ...
  it('文件夹结构 + 大文件 + 空文件：逐文件拉取、哈希校验、结构还原', async () => {
    ...
    writeFileSync(join(src, 'docs', '说明.txt'), '你好，Hive')
    ...
    const outgoing = new Map<string, OutgoingFile>([
      ...
      ['f2', { fileId: 'f2', absPath: join(src, 'docs', '说明.txt'), size: Buffer.byteLength('你好，Hive') }],
    ...
    const plans: IncomingFilePlan[] = [
      ...
      { fileId: 'f2', relPath: '工程/docs/说明.txt', size: Buffer.byteLength('你好，Hive') },
    ...
    expect(readFileSync(join(dst, '工程', 'docs', '说明.txt'), 'utf8')).toBe('你好，Hive')
    expect(readFileSync(join(dst, '工程', 'empty.dat')).length).toBe(0)
    expect(bytes).toBe(big.length + Buffer.byteLength('你好，Hive'))
  }, 15_000)
