...
describe('codec', () => {
  ...
  it('file-ctl 更新包 offer 允许 purpose=update 且不套用群聊图片上限', () => {
    const updateOffer = makeEnvelope<FileCtlPayload>(MSG_TYPES.fileCtl, 'node-aaaa', {
      ...
      seq: 1,
      total: 1,
      files: [{ fileId: 'file-1', path: 'Hive-0.28.0-linux-x64.deb', size: GROUP_IMG_AUTO_ACCEPT + 1 }],
      totalSize: GROUP_IMG_AUTO_ACCEPT + 1,
      fileCount: 1,
      rootName: 'Hive-0.28.0-linux-x64.deb',
      purpose: 'update'
    })
    ...
    const oversized = makeEnvelope<FileCtlPayload>(MSG_TYPES.fileCtl, 'node-aaaa', {
      ...
      seq: 1,
      total: 1,
      files: [{ fileId: 'file-1', path: 'Hive-0.28.0-linux-amd64.deb', size: 512 * 1024 * 1024 + 1 }],
