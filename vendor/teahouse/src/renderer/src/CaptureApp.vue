...
/**
 * 标注色取自 tokens.css 的 Hive 强调红（决议 #281），读一次即缓存。
...
function accent(): string {
  if (!annotationColor) {
    annotationColor =
      getComputedStyle(document.documentElement).getPropertyValue('--primary').trim() || '#e94560'
  }
...
.ann.mosaic {
  background:
    linear-gradient(45deg, rgba(233, 69, 96, 0.35) 25%, transparent 25%) 0 0 / 12px 12px,
    linear-gradient(45deg, transparent 75%, rgba(233, 69, 96, 0.35) 75%) 0 0 / 12px 12px,
...
.btn::before {
  ...
  border-top: 5px solid rgba(12, 15, 22, 0.96);
}
.btn::after {
...
.bar.tooltip-below .btn::before {
  ...
  bottom: auto;
  border-top: 0;
  border-bottom: 5px solid rgba(12, 15, 22, 0.96);
  transform: translate(-50%, -4px);
}
...
.btn.primary:hover {
  /* 主色的按下态沿用项目里 ConvList / App 的做法，不再另写一个主色字面量 */
