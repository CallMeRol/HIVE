...
type Tab = 'chat' | 'contacts' | 'cabinet' | 'network'
...
// Hive #57：网络图节点 / 成员表行 → 打开该成员的机库窗口
async function openMemberHangar(memberId: string): Promise<void> {
  try {
    const result = await window.pantry.openMemberHangar(memberId)
    if (!result.ok) console.warn('[hive] 机库打开失败：', result.reason)
  } catch (err) {
    console.warn('[hive] 机库打开异常：', err)
  }
}
...
function applyWindowTitle(next: SettingsView | null): void {
  ...
  document.title = nick ? `${nick}-Hive` : tr('Hive')
}
  ...
  <div class="shell">
    <nav class="rail">
      ...
      <button
        type="button"
        class="rail-btn rail-hint"
        :class="{ active: tab === 'network', 'show-hint': activeRailHint === 'network' }"
        :data-label="tr('派遣网络图')"
        :aria-label="tr('派遣网络图')"
        @pointermove="scheduleRailHint('network')"
        @pointerleave="hideRailHint('network')"
        @click="activateTab('network', $event)"
      >
        <PantryIcon name="network" :size="25" />
      </button>
    ...
    <aside class="list">
      ...
      <SearchPanel
        v-if="searchQuery.trim() && tab !== 'cabinet' && tab !== 'network'"
        :query="searchQuery.trim()"
        @navigate="((searchQuery = ''), (tab = 'chat'))"
      />
      <CabinetList v-else-if="tab === 'cabinet'" />
      <ConvList v-else-if="tab === 'chat'" />
      <!-- Hive #57：星系派遣网络图占满整个 aside -->
      <DispatchNetwork v-else-if="tab === 'network'" @open="openMemberHangar" />
      <PeerList v-else @select="onSelectPeer" @chat="chatWith" />
    </aside>

    <main class="content">
      <CabinetPane v-if="tab === 'cabinet'" />
      <div v-else-if="tab === 'network'" class="empty">
        <PantryBrandLogo variant="color" :size="92" class="empty-logo" />
        <div class="brand-title">{{ tr('派遣网络图') }}</div>
        <p class="quote">{{ tr('星系图上点一个成员，打开它的机库。') }}</p>
      </div>
      <ProfileCard
      ...
      <div v-else class="empty">
        <PantryBrandLogo variant="color" :size="92" class="empty-logo" />
        <div class="brand-title">{{ tr('Hive') }}</div>
