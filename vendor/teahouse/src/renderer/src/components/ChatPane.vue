...
import HivePendingBar from './HivePendingBar.vue'
import HiveClaimableBar from './HiveClaimableBar.vue'
...
/** Hive #52：机库窗口的失败反馈（headless 恒拒绝、local-api 未就绪都要看得见，不静默）。 */
const hangarFeedback = ref('')
async function openHangar(): Promise<void> {
  hangarFeedback.value = ''
  try {
    const result = await window.pantry.openHangar()
    if (!result.ok) hangarFeedback.value = result.reason ?? tr('暂时无法打开机库')
  } catch {
    hangarFeedback.value = tr('暂时无法打开机库')
  }
}
...
<template>
  ...
  >
    ...
    <header class="head">
      ...
      <!-- Hive #52：独立机库窗口（本机成员过程台）。headless 恒拒绝，见主进程 reason。 -->
      <button v-if="isGroup" class="head-btn" :title="tr('机库')" :aria-label="tr('机库')" @click="openHangar">
        <PantryIcon name="folder-tree" :size="17" />
      </button>
    </header>

    <!-- Hive #50：pending 收集期药丸 + 会话目标状态（目标未定 / 已冻结） -->
    <HivePendingBar
      v-if="isGroup && group"
    ...
    <!-- Hive #59：runner 猝死 / 派遣失败留下的待领取任务 + 人工补派（默认不自动补派） -->
    <HiveClaimableBar v-if="isGroup && group" :group-id="group.groupId" />

    <p v-if="hangarFeedback" class="screen-status" role="status">{{ hangarFeedback }}</p>
...
.jump-latest:hover {
  background: var(--material-strong);
  border-color: var(--primary);
  box-shadow: var(--highlight-edge), 0 8px 24px rgba(0, 0, 0, 0.14);
  opacity: 1;
}
.tool {
  border: none;
...
.history-day.today:not(.range-edge) {
  box-shadow: inset 0 0 0 1px rgba(233, 69, 96, 0.38);
  color: var(--primary);
}
...
.head {
  ...
  border-bottom: 1px solid var(--line);
  box-shadow: var(--highlight-edge), 0 12px 30px rgba(0, 0, 0, 0.035);
  position: relative;
  z-index: 4;
}
.title {
