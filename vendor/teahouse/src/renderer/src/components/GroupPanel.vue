...
import { isHiveMemberRemoveDenied } from '../../../shared/hive-member'
...
// #49：接入对话框**懒加载**（与 MessageRow 的 MessageBody 同法）——主窗静态闭包有硬
// 字节预算，只有点「接入本机 Agent」时才需要这份代码。
//
// 关键：该组件**不得 import store**（群列表由 props 传进来）。懒加载组件若与主闭包
// 共享模块，Rollup 会把 App.vue 的 facade 并进匿名共享块，manifest 里就没有
// `src/App.vue` 这个动态入口了，`check-renderer-bundles.mjs` 的四入口契约随之失效
// （注释里记着这条 landmine）。
const AgentAttachDialog = shallowRef<Component | null>(null)
void import('./AgentAttachDialog.vue').then((mod) => {
  AgentAttachDialog.value = markRaw(mod.default)
})
...
// #49：接入本机 agent 的对话框开关
const showAttachDialog = ref(false)

/**
 * 接入成功后刷新群成员表：新成员是由**另一个节点进程**加进来的，本节点的群信息
 * 要等一次同步才看到它。这里显式拉一次，避免用户以为「接入没生效」。
 */
async function onAgentAttached(): Promise<void> {
  try {
    await window.pantry.getGroup(props.group.groupId)
  } catch {
    // 刷新失败不影响接入本身：底座的下一次 group info 广播会补上。
  }
}
...
/**
 * 接入对话框的群选择项由**这里**算好传下去（见上方懒加载注释：那个组件不许碰 store）。
 * 群数据本来就经 groupsStore 到了本组件，这里只是换个形状。
 */
const groupPickerOptions = computed(() =>
  Object.values(groupsStore.byId).map((g) => ({ label: g.name, value: g.groupId }))
)
...
async function runUpdate(patch: GroupPatch, failureMessage: string): Promise<boolean> {
  ...
  try {
    ...
    // Hive #53：移出被拒时主进程回明确原因（不是含混的失败），直接展示。
    if (isHiveMemberRemoveDenied(updated)) {
      adminFeedback.value = tr(updated.reason)
      return false
    }
    if (!updated) {
...
<template>
  <aside class="panel">
    ...
    <template v-if="group.amMember">
      ...
      <!-- #49：接入本机已有的 Claude Code / Codex 作为成员（与「邀请已有联系人」并列） -->
      <button
        class="add"
        :disabled="adminBusy || atMemberCap"
        :title="tr('接入本机 Agent')"
      ...
      >
        <PantryIcon name="plus" :size="14" />{{ tr('接入本机 Agent') }}
      </button>
      <button
        v-if="canShowAdmin"
        class="add"
    ...
    />
    <!-- #49：接入本机 agent 成员（成功后刷新群成员表，新成员即出现在上方列表） -->
    <component
      :is="AgentAttachDialog"
      v-if="showAttachDialog && group.amMember && AgentAttachDialog"
      :group-id="group.groupId"
      :group-options="groupPickerOptions"
      @close="showAttachDialog = false"
      @attached="onAgentAttached"
    />
    <GroupTextDialog
      v-if="textDialogKind"
      :group="group"
