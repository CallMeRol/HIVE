...
describe('图片原生文字层布局', () => {
  it('按阅读顺序保留原文，将 19px 原生选区双轴拟合到 16px 行框', () => {
    ...
    const boxes = getImageTextLineBoxes([line('Hive OCR', 1), line('AB  CD', 0)], 400, 300)
    const result = layoutImageTextLines(boxes, measure)
    expect(result.map((item) => item.text)).toEqual(['AB  CD', 'Hive OCR'])
  ...
  it('补偿实际文字盒的左右和基线偏移，行高变化后仍贴合目标位置', () => {
    const boxes = getImageTextLineBoxes([line('Hive OCR', 5)], 400, 300)
