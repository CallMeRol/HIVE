...
import {
  ...
  type AttachRequest,
  type AttachResult,
  type AttachRuntimeProbe,
  type AttachedMemberView,
  ...
  type GroupPatch,
  type GroupView,
  type HangarOpenResult,
  type HiveClaimableState,
  type HiveClaimResult,
  type HiveClearResult,
  type HiveReclaimResult,
  type HiveNetworkSnapshot,
  type HiveMemberRemoveResult,
  type HivePendingState,
  type HivePendingResult,
  type HivePendingUpdateData,
  type ImageOcrResult,
  type ImageOcrSource,
...
const api: PantryApi = {
  ...
  // Hive #53：remove 被拒时主进程回 HiveMemberRemoveDeny（而不是含混的 null）。
  ...
  // Hive #52：独立机库窗口 + clear 命令面。
  openHangar: (): Promise<HangarOpenResult> => ipcRenderer.invoke(IpcChannels.hiveOpenHangar),
  clearHiveMember: (memberId: string): Promise<HiveClearResult> =>
    ipcRenderer.invoke(IpcChannels.hiveClear, memberId),
  // Hive #54：回收派遣成员（输出交接校验通过才放行）。
  reclaimHiveMember: (memberId: string, handoverMarkdown: string): Promise<HiveReclaimResult> =>
    ipcRenderer.invoke(IpcChannels.hiveReclaim, memberId, handoverMarkdown),
  // Hive #57：派遣网络图数据面 + 成员机库直达。
  getHiveNetwork: (): Promise<HiveNetworkSnapshot> => ipcRenderer.invoke(IpcChannels.hiveNetwork),
  openMemberHangar: (memberId: string): Promise<HangarOpenResult> =>
    ipcRenderer.invoke(IpcChannels.hiveOpenHangar, memberId),
  // Hive #59：待领取任务查询 + 人工补派（默认不自动补派，这是唯一补派入口）。
  listClaimable: (): Promise<HiveClaimableState> =>
    ipcRenderer.invoke(IpcChannels.hiveClaimableList),
  claimNow: (dispatchId: string): Promise<HiveClaimResult> =>
    ipcRenderer.invoke(IpcChannels.hiveClaimNow, dispatchId),
  setGroupAvatar: (
    groupId: string,
  ...
  getPendingState: (): Promise<HivePendingState | null> =>
    ipcRenderer.invoke(IpcChannels.hivePendingState),
  confirmPending: (memberId: string, goalText?: string): Promise<HivePendingResult> =>
    ipcRenderer.invoke(IpcChannels.hivePendingConfirm, memberId, goalText),
  dropPending: (memberId: string): Promise<HivePendingResult> =>
    ipcRenderer.invoke(IpcChannels.hivePendingDrop, memberId),
  grantConfirm: (targetId: string, grant: boolean): Promise<HivePendingResult> =>
    ipcRenderer.invoke(IpcChannels.hivePendingGrant, targetId, grant),
  freezeGoal: (memberId: string, groupId: string, text: string): Promise<HivePendingResult> =>
    ipcRenderer.invoke(IpcChannels.hiveGoalFreeze, memberId, groupId, text),
  onPendingUpdated: (listener: (data: HivePendingUpdateData) => void): (() => void) =>
    subscribe(IpcEvents.pendingUpdated, listener),
  /** #59：待领取任务变化推送（runner 猝死 / 人工补派后刷新面板）。 */
  onClaimableUpdated: (listener: () => void): (() => void) =>
    subscribe(IpcEvents.claimableUpdated, listener),
  // #49 接入面：入参同样要剥掉响应式代理，否则结构化克隆会抛。
  probeAttachTargets: (): Promise<AttachRuntimeProbe[]> =>
    ipcRenderer.invoke(IpcChannels.agentProbe),
  attachAgent: (request: AttachRequest): Promise<AttachResult> =>
    ipcRenderer.invoke(IpcChannels.agentAttach, JSON.parse(JSON.stringify(request))),
  listAttachedAgents: (): Promise<AttachedMemberView[]> =>
    ipcRenderer.invoke(IpcChannels.agentAttachList),
