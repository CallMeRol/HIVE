...
import type { HiveMemberRemoveResult } from './hive-member'
import type { HangarOpenResult, HiveClearResult, HiveReclaimResult } from './hive-hangar'
import type { HiveClaimableState, HiveClaimResult } from './hive-claimable'

/** Hive #50 pending 视图（GUI 药丸 + 目标状态徽标的数据源）。 */
export interface HivePendingView {
  memberId: string
  groupId: string
  entries: Array<{ senderId: string; text: string; ts: number }>
  createdAt: number
}

/** 会话目标状态（未定 / 已冻结；PRD ②「目标还没冻结时，守卫放行」）。 */
export interface HiveGoalView {
  memberId: string
  groupId: string
  state: 'unset' | 'frozen'
  text: string
...
export interface HivePendingState {
  pendings: HivePendingView[]
  goals: HiveGoalView[]
  /** 被放权者（除主人外可按确认键的人）。 */
  confirmGrants: string[]
  /** 本机节点 id（渲染层区分「我的 pending」）。 */
  selfId: string
}

/** Hive #50 动作返回：ok = 成功；deny = 权限/状态拒绝（code + reason 显式）。 */
export interface HivePendingResult {
  ok: boolean
  code?: string
  reason?: string
}

/** `hive:pending-updated` 推送 data（与 SSE `pending.updated` 同一形状）。 */
export interface HivePendingUpdateData {
  memberId: string
  goal: string
  entries: number
...
export const IpcChannels = {
  ...
  /** Hive #52：独立机库窗口（加载 local-api 的静态机库页面）+ clear 命令面。 */
  hiveOpenHangar: 'hive:open-hangar',
  hiveClear: 'hive:clear',
  /** Hive #54：回收派遣成员（输出交接校验通过才放行；leave + 硬删 + 归档 + quit + 释放 slot）。 */
  hiveReclaim: 'hive:reclaim',
  /** Hive #57：派遣网络图数据面（活跃成员 + 派遣关系 + 负担快照）。 */
  hiveNetwork: 'hive:network',
  /** Hive #59：待领取任务（runner 猝死/派遣失败）查询与人工补派命令面。 */
  hiveClaimableList: 'hive:claimable-list',
  hiveClaimNow: 'hive:claim-now',
  ...
  hivePendingState: 'hive:pending-state',
  hivePendingConfirm: 'hive:pending-confirm',
  hivePendingDrop: 'hive:pending-drop',
  hivePendingGrant: 'hive:pending-grant',
  hiveGoalFreeze: 'hive:goal-freeze',
  /** #49 接入：探测本机 runtime（模型 / permission 枚举 + 可操作诊断） */
  agentProbe: 'agent:probe',
  /** #49 接入：把一个本机 agent 接成群成员 */
  agentAttach: 'agent:attach',
  /** #49 接入：列出本机已接入的成员 */
  agentAttachList: 'agent:attach-list',
...
export const IpcEvents = {
  ...
  /** Hive #50：pending 收集期变化（药丸与目标状态徽标的推送面） */
  pendingUpdated: 'hive:pending-updated',
  /** Hive #59：待领取任务变化（runner 猝死 / 补派后的推送面） */
  claimableUpdated: 'hive:claimable-updated'
} as const
...
/** 上下文负担五档（sprite 契约 A2 / #26）：idle 蓝 / easy 绿 / busy 黄 / overload 橙 / about-to-blow 红。 */
export const BURDEN_TAGS = ['idle', 'easy', 'busy', 'overload', 'about-to-blow'] as const
export type BurdenTag = (typeof BURDEN_TAGS)[number]

/**
 * 对端负担面（Hive 私有，跨机 agent-status 通道投影；#47 / ADR-0009）。
 * `eligible=false` = 结构性无状态的对端（不在同群 / 未声明 ag1 / 旧版本），
 * 渲染层保留底座绿/灰点，**不**落 idle 蓝 —— 「永远不会有」≠「空闲」。
 */
export interface PeerBurdenView {
  eligible: boolean
  tag: BurdenTag
  /**
   * 档名的展示文案（`空闲`/`轻松`/…），由主进程按 #26 口径给出。
   * 放在视图模型里而不是渲染层：`src/i18n/en.ts` 与 `PeerList.vue` 的静态中文
   * 分别受上游 `i18n/coverage.test.ts` 约束，两者都不在补丁面白名单内、
   * 不得新增中文字面量；渲染层只做透传。
   */
  label: string
  /**
   * 该档的固定色相（sprite 契约 A2 hex，= 塔奇克马五套 GIF 配色）。
   * 由主进程透传、渲染层直接内联：色值表不进主窗 CSS 闭包（有硬预算）。
   */
  color: string
  /**
   * 状态点的完整悬浮说明（档名 + 占用 % + 窗口大小）已由主进程拼好。
   * 渲染层只透传：主窗静态闭包有硬预算，且渲染层不得新增中文字面量。
   */
  title: string
  /** 窗口占用百分比；无有效数据（stale）为 null，不假装成 0。 */
  pct: number | null
  /** 窗口大小（tokens）；必须与 pct 同屏，同为 10% 的 200K 与 1M 窗口处境完全不同。 */
  sizeTokens: number | null
  /** 当前会话目标（随包携带，收方不重跑目标抽取器）。 */
  goal: string
  /** 由目标派生的临时角色。 */
  role: string
  /** 最后活动 = 最后一次有效上报的 ts；从未上报为 0。 */
  ts: number
  /** TTL（30s）内没数据 = true；拿不到数据落 idle 蓝但继续显示（#17 Q3「没停顿」）。 */
  stale: boolean
}
...
export interface PeerView {
  ...
  caps: string[]
  /**
   * 上下文负担面（Hive 私有；#47）。缺省 = 本节点不跑 agent 状态通道，
   * 渲染层一律退回底座绿/灰点。
   */
  burden?: PeerBurdenView
  /**
   * Hive #57：成员表 / 派遣网络图的派遣面（目标、派遣者、深度、主人、权限）。
   * 缺省 = 底座节点（无 Hive 登记信息），UI 按普通成员渲染。
   */
  hive?: PeerHiveView
}

/** Hive #57：单成员的派遣关系投影（主进程拼装，渲染层只透传）。 */
export interface PeerHiveView {
  /** 当前会话目标（一行话）；空 = 未定。 */
  goal: string
  /** 派遣者 = 当前活跃挂靠父（member-registry.spawnedBy）；空 = 常驻或主人即主人。 */
  dispatcherId: string
  /** 派遣深度（活跃链代数；常驻 = 1）。 */
  depth: number
  /** 成员类别：常驻 / 派遣（member-registry.kind）。 */
  kind: 'resident' | 'dispatched'
  /** 主人（人对成员的成员级关系）。 */
  ownerId: string
  /** 群内权限（底座 GroupView 投影）：owner / admin / member。 */
  role: 'owner' | 'admin' | 'member'
}

/** Hive #57：派遣网络图快照（节点 = 本机可见的全部活跃成员）。 */
export interface HiveNetworkSnapshot {
  /** 派遣深度上限与并发上限（图例文案 + 演示叙事用）。 */
  maxDepth: number
  maxConcurrency: number
  nodes: HiveNetworkNode[]
}

/** 网络图节点 = PeerView 投影 + 活跃挂靠边。 */
export interface HiveNetworkNode {
  nodeId: string
  name: string
  online: boolean
  burden?: PeerBurdenView
...
// ——— #49 Agent 接入面 ———
// 渲染层只消费这些投影，不碰文件系统、不认识 adapter 的包名。

/** 支持的 ACP runtime。 */
export const ATTACH_RUNTIMES = ['claude', 'codex'] as const
export type AttachRuntime = (typeof ATTACH_RUNTIMES)[number]

/** 探测项：失败必须带 `hint`（可操作建议），这是 AC2 的对外形状。 */
export interface AttachProbeItem {
  id: string
  label: string
  ok: boolean
  detail: string
...
/** 一个可选项（模型 / permission 档位），来自 adapter 自报，不硬编码。 */
...
export interface AttachRuntimeProbe {
  runtime: AttachRuntime
  ready: boolean
  items: AttachProbeItem[]
  adapterPath: string
  cliPath: string
  models: AttachOption[]
  permissions: AttachOption[]
  agentInfo?: { name: string; version: string }
}

/** 接入请求：AC1 的五个可选项 + 探测给出的 adapter 路径。 */
export interface AttachRequest {
  ...
  groupId: string
  /** 探测结果里的 adapter 绝对路径；空 = 让主进程按 runtime 解析。 */
  adapterPath: string
}

/** 接入结果：成功给身份与端口，失败给可操作建议。 */
...
/** 已接入成员（GUI 列表）。 */
export interface AttachedMemberView {
  memberId: string
  runtime: AttachRuntime
  nick: string
  cwd: string
...
export const SHARE_ROOT_REJECT_TEXT: Record<ShareRootRejectReason, string> = {
  ...
  'app-data': '不能共享Hive自己的数据目录，请换一个文件夹',
...
export interface PantryApi {
   ...
   *
   * **Hive #53**：`remove` 多一层「成员主人」判定（CONTEXT「移出」= 仅**该成员的主人**可做）。
   * 拒绝时返回 `HiveMemberRemoveDeny`（失败原因显式、可展示），而不是含混的 `null`；
   * 其余 kind 与上游同形（`GroupView | null`）。
   */
  updateGroup(groupId: string, patch: GroupPatch): Promise<HiveMemberRemoveResult | null>
  // ---- Hive #50：pending 收集期 + 确认键 + 会话目标 ----
  /** 当前 pending 面（本机视角）；未启用 pending 闸门（PANTRY_PENDING≠1）返回 null。 */
  getPendingState(): Promise<HivePendingState | null>
  /** 确认键：聚合为一条 prompt 发送并冻结会话目标；拒绝返回 deny（fail-closed）。 */
  confirmPending(memberId: string, goalText?: string): Promise<HivePendingResult>
  /** 主人丢弃 pending（唯一出口之二）。 */
  dropPending(memberId: string): Promise<HivePendingResult>
  /** 主人逐人放权 / 收权（给的是「按」的权力，不免审）。 */
  grantConfirm(targetId: string, grant: boolean): Promise<HivePendingResult>
  /** 主人修订并冻结会话目标（独立于确认键的入口）。 */
  freezeGoal(memberId: string, groupId: string, text: string): Promise<HivePendingResult>
  /** `hive:pending-updated` 推送订阅（药丸 + 目标状态）。 */
  onPendingUpdated(listener: (data: HivePendingUpdateData) => void): () => void
  /**
   * **Hive #52**：打开独立机库窗口（加载 local-api 的静态机库页面）。
   * headless 节点恒 `ok:false` + 显式 reason（ADR-0002：不建该窗口）。
   */
  openHangar(): Promise<HangarOpenResult>
  /**
   * **Hive #52**：clear 一个成员实例——旧实例归档、同一身份位置开新实例、目标由人重定。
   * 权限与 #53 同源（仅该成员的主人可做），拒绝返回显式 reason。
   */
  clearHiveMember(memberId: string): Promise<HiveClearResult>
  /**
   * **Hive #54**：回收一个派遣成员——输出交接校验（七节 + 产物 sha256，fail-closed）
   * 通过后 leave、从成员表与登记表硬删、机库归档、quit、释放 slot。仅派遣者可发起。
   */
  reclaimHiveMember(memberId: string, handoverMarkdown: string): Promise<HiveReclaimResult>
  /** **Hive #57**：派遣网络图数据面（星系图 + 成员表派遣列共用）。 */
  getHiveNetwork(): Promise<HiveNetworkSnapshot>
  /** **Hive #57**：打开某成员的机库窗口（点击成员表行 / 网络图节点）。 */
  openMemberHangar(memberId: string): Promise<HangarOpenResult>
  /**
   * **Hive #59**：当前待领取任务（派遣失败 / runner 猝死 / 重启中断留下的）。
   * 未启用派遣账本时返回 `tasks: []`（不是错误：没有账本就没有待领取任务）。
   */
  listClaimable(): Promise<HiveClaimableState>
  /**
   * **Hive #59**：人工补派一条待领取任务。默认**不**自动补派，这是唯一的补派入口。
   * 权限 = 该任务的原主人（dispatcherId）；拒绝返回显式 reason。
   */
  claimNow(dispatchId: string): Promise<HiveClaimResult>
  /** `hive:claimable-updated` 推送订阅（补派 / 猝死后刷新面板）。 */
  onClaimableUpdated(listener: () => void): () => void
  ...
  /** #49：探测本机可接入的 runtime（含模型 / permission 枚举与可操作诊断）。 */
  probeAttachTargets(): Promise<AttachRuntimeProbe[]>
  /** #49：把一个本机 agent 接成群成员。失败时返回 ok=false + `hint`，不抛。 */
  attachAgent(request: AttachRequest): Promise<AttachResult>
  /** #49：列出本机已接入的成员。 */
  listAttachedAgents(): Promise<AttachedMemberView[]>
  /** 触发截图（等价全局快捷键） */
...
// Hive #53：成员移出拒绝码的三面共用出口（主进程判定 / preload 透传 / renderer 文案）。
export { hiveMemberRemoveText, isHiveMemberRemoveDenied } from './hive-member'
export type {
  HiveMemberRemoveDeny,
  HiveMemberRemoveDenyCode,
  HiveMemberRemoveResult
} from './hive-member'

// Hive #52：机库命令面（打开窗口 / clear）的返回形状出口。
export { HANGAR_OPEN_REASON, HIVE_CLEAR_TEXT } from './hive-hangar'
export type { HangarOpenResult, HiveClearResult, HiveReclaimResult } from './hive-hangar'

// Hive #59：待领取任务的 GUI 面出口（查询 + 人工补派）。
export { HIVE_CLAIM_TEXT, toClaimableView } from './hive-claimable'
export type { HiveClaimableView, HiveClaimableState, HiveClaimResult } from './hive-claimable'
