...
// 两个版本（决议 #58/#107/#226，Hive 品牌源）：
//   mono  —— macOS 菜单栏 template，由 build/icons/hive-tray-mono.png 缩放
//   color —— Windows / Linux 彩色版，由 build/icons/pantry-logo-icon.png 缩放
// 依赖 macOS sips；运行态仍只使用内嵌 Data URL，不依赖外部文件。
...
try {
  const mono = renderPng(join(iconDir, 'hive-tray-mono.png'), tmp)
